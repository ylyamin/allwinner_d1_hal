...
{
	CCU->DSI_CLK_REG = (1 << 24) | (0 << 8) | (4 << 0); //! orig 1 - pll, 2 - video, M = 0
...
static void ccu_peri0_pll_set(void)
{
	// default AUDIO1 3072MHz
	if(!(CCU->PLL_PERI_CTRL_REG & BV(31))) {
		CCU->PLL_PERI_CTRL_REG |= BV(31) | BV(30); // enable pll, ldo
