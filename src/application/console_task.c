...
uint16_t console_new_line_buf[BUFF_SIZE];
...
void console_task_init(void)
{
    ...
    fifo_init(&console_new_line_buf_fifo, console_new_line_buf, sizeof(uint16_t), sizeof(console_new_line_buf));
...
void console_new_line_buf_write(uint16_t elem)
{
...
uint16_t console_new_line_buf_read(void)
{
    uint32_t * read_addr = fifo_get_write_addr(&console_new_line_buf_fifo);
    return *read_addr;
}
...
void console_render(void)
{
    ...
    if(fifo_empty(&console_string_buf_fifo) != 1)
    {
        ...
        //first run, 
        if(console_string_buf_fifo.read == 0) {
            need_rerender = 0;
            console_new_line_buf_write(0);
        }
        ...
        while(fifo_get_read_size_cont(&console_string_buf_fifo))
        {
            uint8_t symbol;
            if(need_rerender) {
                //if need_rerender != read and > write
                //symbol = *(need_rerender++)
            ...
            if((shift_y / ch_size) == row_num) { ////end screen
                 shift_y = 0; 
                 //need_rerender = fifo_get_write_addr(&console_new_line_buf_fifo) - row_num;
            }
        }
    }
}